...
- 通过数据库迁移扩容 `supervisor.password` 以保存 PBKDF2 哈希；既有账号首次成功登录时原位升级，不保存、不返回或记录新的明文密码。
...
[
  ...
  },
  {
    "id": "AC-003",
    "category": "security",
    "description": "密码在成功登录后以 PBKDF2 哈希形式存储和比对，登录响应、日志、测试和前端状态均不包含明文密码或密码哈希。",
    "steps": ["检查 supervisor 迁移、认证服务、接口 DTO 与响应对象。", "验证既有明文仅在首次成功认证后原位升级为哈希，且密码字段未被返回或打印。", "验证认证查询与密码比较在服务层完成。"],
    "passes": true
  },
  {
    "id": "AC-004",
...
- 新增数据库迁移，扩容 `supervisor.password` 以保存密码哈希；用户于 2026-09-08 在确认实施方案时已授权。

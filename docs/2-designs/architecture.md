...
控制器处理路由与请求/响应；服务层承载业务逻辑；Mapper 与 `JdbcTemplate` 执行数据访问；`ResultVO` 统一封装接口响应。前端 API 客户端集中在 `front/src/api/`，视图位于 `front/src/views/`。NEPS 登录由认证控制器接收请求、认证服务查询 `supervisor` 并校验 PBKDF2 密码哈希，成功后仅在 HTTP Session 写入监督员手机号和姓名等非敏感身份信息。
...
- 当前 NEPS 登录 MVP 不引入全局拦截器或 JWT；Session 仅用于建立登录态，后续反馈归属隔离将在独立任务中实施。
